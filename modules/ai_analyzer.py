...
- 槓桿倍數由你自行決定（1-50x），根據行情波動和信心程度調整：
  - 趨勢明確、信心高 → 可用高槓桿（30-50x）
  - 盤整或方向不確定 → 用低槓桿（10-20x）
  - 高波動時段（重大數據前後）→ 降低槓桿
- 每筆倉位由你自行決定（1-5%），必須同時考慮信心程度、止損距離和槓桿倍數：
...
- 例如：50x 往返成本 ≈ 5.0%（佔保證金），25x ≈ 2.5%，10x ≈ 1.0%
...
17. 主動平倉（CLOSE）：當持倉方向與最新分析師觀點明顯反轉、或技術面出現趨勢結構破壞時，不要等止損觸發，主動用 CLOSE 平倉。CLOSE 優先於 ADJUST — 如果情況嚴重到 SL/TP 調整已無法挽救，直接平倉
...
### 決策類型 3：緊急平倉（CLOSE）
如果已有持倉，且出現以下情況之一，應立即平倉：
- 分析師觀點完全反轉（原本看多現在看空，或反之）
- 技術面出現明確反轉信號（趨勢結構破壞）
- 重大利空/利多消息與持倉方向相反
- 持倉方向與多時間框架對齊方向嚴重背離
...
{{
  ...
  "trade_id": 要平倉的交易 ID,
  "symbol": "交易對",
  "confidence": 0-100（對平倉決策的信心）,

  "reasoning": {{
    "analyst_consensus": "分析師最新觀點",
    "technical": "技術面反轉信號",
    "close_reason": "為什麼需要緊急平倉（具體原因）"
  }}
}}
...
{{
  ...
  "stop_loss_assessment": "停損設定是否合理（位置是否在關鍵支撐/阻力位？距離是否適當？）",
...
### 決策類型 3：緊急平倉（CLOSE）
如果已有持倉方向與最新分析師觀點、技術面、MTF 對齊方向嚴重背離，應立即平倉止損：
