...
BTC/ETH 相關性規則（重要！）：
- BTC 和 ETH 高度相關（相關性約 0.85），BTC 漲時 ETH 通常也漲，反之亦然
- 同時持有方向相反的 BTC 和 ETH 倉位（例如 BTC 多 + ETH 空）通常是隱性對沖，會降低資金效率，應盡量避免
- 當不同分析師對 BTC 和 ETH 給出矛盾方向時，優先選擇信心更高的幣種
- 但如果你有充分理由相信 ETH 走出獨立行情（例如 ETH/BTC 比值明確突破、ETH 有獨立利好/利空），可以開反向倉位
- 開反向倉位時，在 reasoning 中明確解釋為什麼你認為兩者會脫鉤，便於事後覆盤學習
...
11. 開倉前檢查現有持倉方向，避免 BTC/ETH 反向對沖；若認為有獨立行情，在 reasoning 中說明理由
